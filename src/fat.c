...
static u64 find_mbr_partition(void) {
    ...
    ahci_read(ahci_sata_port, 0, 1, buf);
...
static u64 find_gpt_partition(void) {
    ...
    ahci_read(ahci_sata_port, 1, 1, buf);
    ...
    u32 sectors_needed = (num_entries + entries_per_sector - 1) / entries_per_sector;

    buf = kalloc(PAGES_FOR(sectors_needed * 512));
    if (!buf) return 0;

    ahci_read(ahci_sata_port, entries_lba, sectors_needed, buf);
    ...
    for (u32 i = 0; i < num_entries; i++) {
        ...
        int is_null = 1;
        for (int j = 0; j < 16; j++) {
            if (entry->type_guid[j] != 0) { is_null = 0; break; }
        }
        if (is_null) continue;
...
void fat_init(void) {
    ...
    ahci_read(ahci_sata_port, partition_lba, 1, boot_sect);
    ...
    if (!fs_info)
        panic("OUT OF MEMORY", 0);
    ahci_read(ahci_sata_port, partition_lba + boot_sect->ebr.fsinfo_sector, 1, fs_info);
...
u32 fat_readdir(u32 dir_cluster, struct fat_dir_entry *out, u32 index) {
    ...
    u32 cluster_size = boot_sect->bpb.sectors_per_cluster * boot_sect->bpb.bytes_per_sector;
    u32 pages = PAGES_FOR(cluster_size);
    u8 *buf = kalloc(pages);
    if (!buf)
        return -1;

    ahci_read(ahci_sata_port, sector,
